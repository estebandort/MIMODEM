...
def augment_and_normalize_image(image, auxiliary_image, view, best_center, random_number_generator,
    ...
    ########################################################################################################################
    if augmentation:
        cropped_image, cropped_auxiliary_image, borders = augmentations.random_augmentation_best_center(
            image=image,
    ...
    return cropped_image, cropped_auxiliary_image, borders #############################################################################
    


def bordbord(image, view, best_center):
    """
    Applies augmentation window with random noise in location and size
    and return normalized cropped image.
    """
    bord = augmentations.borders(
        image=image,
