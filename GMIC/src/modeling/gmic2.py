...
import numpy as np
from src.utilities import tools2
...
class GMIC(nn.Module):
    ...
    def forward(self, x_original):
        ...
        h_g, self.saliency_map, cam_score = self.global_network.forward(x_original) ######################################################

        # calculate y_global
        # note that y_global is not directly used in inference
        # [N,1]
        self.y_global = self.aggregation_function.forward(self.saliency_map)
        # y_score = self.aggregation_function.forward(cam_score)
        y_score = cam_score##########################################################################################################
        ...
        return forward_altercam(self, x_original, activation='xgradcam')
        #return self.y_global, self.y_local, self.y_fusion, self.saliency_map, y_score, h_g


    def forward_global(self, x_original):
        """
        :param x_original: N,H,W,C numpy matrix
        """
        # global network: x_small -> class activation map
        #h_g, self.saliency_map = self.global_network.forward(x_original)
        h_g, self.saliency_map, cam_score = self.global_network.forward(x_original)
        # cam_score = self.aggregation_function.forward(cam_score)

        return h_g, self.saliency_map, cam_score

    def altercam(self, x_original, h_g, cam_score,saliency_map, activation='xgradcam'):
            """
            :param x_original: N,H,W,C numpy matrix
            """
            # global network: x_small -> class activation map
            ...
            y_score = self.aggregation_function.forward(cam_score)
            # if activation == 'xgradcam':
            #     #XgradCam
            #     #xgradcam_extractor = XGradCAM(self.global_network, target_layer='ds_net.relu')  # [works fine]
            #     xgradcam_extractor = XGradCAM(self, target_layer='ds_net.relu')  # [works fine]
            #     self.saliency_map = xgradcam_extractor(class_idx=1, scores=cam_score)[0]
    ...
    def forward_altercam(self, x_original, activation='xgradcam'):
            """
            :param x_original: N,H,W,C numpy matrix
            """
            # global network: x_small -> class activation map
            ...
            if activation == 'xgradcam':
                #XgradCam
                #xgradcam_extractor = XGradCAM(self.global_network, target_layer='ds_net.relu')  # [works fine]
                xgradcam_extractor = GradCAM(self, target_layer='ds_net.relu')  # [works fine]
                #self.saliency_map = xgradcam_extractor(class_idx=1, scores=y_score)[0]
                self.saliency_map = xgradcam_extractor(class_idx=1, scores=cam_score)[0]
